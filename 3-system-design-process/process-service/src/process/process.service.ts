...
export class ProcessService {
    ...
    estimate(dau: number) {
        ...
        const requestsPerUser = 20 // Giả định mỗi user thực hiện 20 requests/ngày (EN: Assume 20 requests/day per user)
        const qps = Math.ceil((dau * requestsPerUser) / (24 * 3600))
        const peakQps = qps * 5
        const storagePerDayGb = (dau * 0.5) / 1024 // Giả định 0.5MB/user/day
        ...
        return {
            ...
            conclusion: peakQps > 1000 ? "Recommendation: Horizontal Scaling + Load Balancer required" : "Recommendation: Vertical Scaling might be enough"
        }
    }
