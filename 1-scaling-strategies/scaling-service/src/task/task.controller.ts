...
export class TaskController {
     ...
     *
     * Dùng để minh họa sự khác biệt khi nâng cấp tài nguyên (Vertical)
     * hoặc tăng số lượng node (Horizontal)
     * (EN: Used to demonstrate difference when upgrading resources (Vertical)
     * or increasing node count (Horizontal))
     */
    @Get("heavy")
    handleHeavyTask(@Query("load") load: string) {
        const iterations = parseInt(load) || 10000000
